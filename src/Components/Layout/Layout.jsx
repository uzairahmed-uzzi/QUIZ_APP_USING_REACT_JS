...
const Layout = () => {
    ...
    const increaseNo=()=>setQNo((prev)=>prev+1);   
    // setData(()=>JSON.parse([
    //     {
    //       id: 505,
...
const getQuestions=async ()=>{
    const response=await Axios.get("https://quizapi.io/api/v1/questions?apiKey=5ZneGMpA3dgdpWS1JyUEWTpNrqcRhJcS8WbQX2if&category=code&difficulty=Easy&limit=10");
...
useEffect(()=>{
    ...
    <div className='container'>
        ...
        {data[qNo].answers.map((ele,ind)=>{
         <Option option={ele} key={ind} next={increaseNo}/>   
        })}
        </div>
